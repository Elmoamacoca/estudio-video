...
/* ------------------------------------------ A INSISTÊNCIA PELA IDENTIFICAÇÃO
   ...
   TRÊS FREIOS, porque cada tentativa deixa duas gravações no acervo:
     1. só com a aba à vista, para uma janela esquecida não gravar a noite inteira;
     2. teto de vinte e cinco tentativas por perfil, e depois disso ele espera o Iniciar;
     3. uma de cada vez, sem empilhar chamada em cima de chamada. */
const TENTATIVAS = new Map();
const TETO_TENTATIVAS = 25;
let insistindo = false;

async function insistirIdentificacao() {
  if (insistindo || document.visibilityState !== "visible") return;
