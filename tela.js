...
const pesoDe = e => PESO[e.tipo] || (e.tipo ? "evento" : (e.gravidade || "evento"));
  ...
  const html = pedaco.map(bruto => {
    ...
    const c = b && b.quando > (bruto.ultimo || 0)
      ? { ...bruto, lidos: b.lidos, publicacoes: b.publicacoes || bruto.publicacoes,
          modo: b.modo, completo: b.completo, ultimo: b.quando, vivo: !b.completo }
      : bruto;
...
function desenhaProntos(sel, perfis) {
  ...
  const baixaveis = itens.filter(i => i.formato === "reels" && i.arquivo).length;
  const acimaTotal = perfis.reduce((a, b) => a + (b.acima || 0), 0);

  /* UM LOTE SÓ, COM OS MELHORES DE TODOS OS PERFIS.
     Havia um botão por perfil escrito "Baixar 254 reels", e ele mentia: quem baixa é
     um trabalho que percorre a SELEÇÃO INTEIRA, ordenada do maior índice para o menor,
     sem separar por conta. Apertar no @blankpartners trazia os melhores de todo mundo.
     Agora o botão diz o que acontece, e a escolha que existe de verdade é quantos. */
  const nome = rotuloDosFormatos(null, LIVRO[0] || null);
  alvo.innerHTML = `
    <div class="caixa lote-montar">
      <h3>Montar um lote</h3>
      <p class="nota">Os arquivos saem em ordem de desempenho, do maior múltiplo da
        mediana para o menor, juntando todos os perfis. Só reels têm arquivo de vídeo:
        imagem e carrossel entram na lista, mas não têm o que baixar.</p>
      <div class="lote-linha">
        <label class="campo-rot" for="lote_quantos">Quantos arquivos neste lote</label>
        <input type="number" id="lote_quantos" value="${Math.min(150, baixaveis) || 50}"
               min="1" max="500" step="10">
        <button class="acao forte" id="lote_vai" ${baixaveis ? "" : "disabled"}>
          Montar lote</button>
      </div>
      <p class="nota" id="recado_lote">${baixaveis
        ? `${num(baixaveis)} reels com arquivo disponível, de ${num(acimaTotal)} acima da régua.`
        : "Nenhum reels com arquivo disponível: os links vencem em algumas horas, e a esteira renova ao varrer de novo."}</p>
    </div>
    <div class="prontos-lista">${perfis.map(p => {
      const meus = itens.filter(i => i.conta === p.conta);
      const acima = p.acima != null ? p.acima : meus.filter(i => i.indice >= corte).length;
      return `<div class="pronto">
        <div class="pronto-topo"><span class="quem">@${p.conta}</span></div>
        <div class="pronto-numeros">
          <div><b>${num(p.lidos)}</b><span>${nome} varridos</span></div>
...
function desenhaLotes(l) {
  ...
  $("lotes").innerHTML = l.map(x => `<div class="pronto">
    <div class="pronto-topo">
      ...
      <a class="acao forte" target="_blank" rel="noopener"
         href="https://github.com/${DONO}/${REPO}/actions/runs/${x.execucao}">Baixar o ZIP</a>
    </div>
    <div class="pronto-numeros">
      <div><b>${num(x.arquivos)}</b><span>arquivos</span></div>
      <div><b>${x.mb} MB</b><span>tamanho</span></div>
      <div><b>${quando(x.quando)}</b><span>montado</span></div>
    </div>
    <p class="nota mini">${x.criterio || "sem critério anotado"} · o arquivo fica
      guardado por 14 dias e abre na página da execução, no fim, em "Artifacts".</p>
    </div>`).join("");
}
...
document.addEventListener("click", async ev => {
  ...
  const b = $("lote_vai");
  b.disabled = true;
  const quantos = Math.max(1, parseInt($("lote_quantos").value, 10) || 50);
  // baixar um lote leva minutos, e é o carregamento mais longo do sistema
  carregando("recado_lote", `Montando o lote com ${quantos} arquivos`, "bolas");
  try {
    // só o tamanho vai daqui. Formato e corte vêm da régua gravada no acervo, a mesma
    // escolhida ao iniciar: mandar outra por aqui criaria duas verdades.
    await mandar("/baixar", { quantos });
    parado("recado_lote", "lote em preparo. Ele aparece em Lotes montados ao terminar, "
