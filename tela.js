...
document.addEventListener("click", async ev => {
  ...
  // COM O POSTO DE PÉ NÃO HÁ PASTA A PEDIR: quem enxerga o disco é ele. A caixinha de
  // permissão só volta a aparecer no plano B, quando o posto está fora.
...
async function lerAcervo() {
  ...
  if (await postoDePe()) {
    try {
      ...
      return true;
    } catch (e) { ACERVO.itens = []; return null; }
  }
...
async function desenhaAsPastas() {
  ...
  if (!EDIT_RAIZ) {
    ...
    lista.innerHTML = '<div class="cfg-linha"><div><div class="cfg-nome">'
      + 'O posto está fora e a pasta não foi liberada</div><div class="cfg-desc">Espere '
      + 'um minuto e recarregue a página; ou abra a aba de Edição e libere a pasta do '
      + 'Estúdio, que é o plano B para o navegador enxergar o disco.</div></div></div>';
    return;
  }
