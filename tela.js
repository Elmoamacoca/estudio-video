...
function linhaDoAgora(b) {
  ...
  ln.innerHTML = `<i></i><span class="oque"><b></b>`
    + `<span class="dados"><span><b>${num(b.lidos)}</b>${
        b.publicacoes ? ` de ${num(b.publicacoes)}${b.modo === "reels" ? " reels" : ""}`
                      : " posts lidos"}</span>`
...
async function aoVivo() {
  ...
  if (d && !d.erro) {
    ...
    if (batendo.length) {
      ...
      const emReels = batendo.every(b => b.modo === "reels");
      ...
      resumo = batendo.map(b => `@${b.conta}: ${num(b.lidos)}`
        + (b.publicacoes
            ? ` de ${num(b.publicacoes)} ${b.modo === "reels" ? "reels" : "posts"}`
            : " posts lidos")).join(" · ");
